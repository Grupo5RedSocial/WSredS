		...
		while (rs.next()) {
			...
			if (idPAnt != idPubAct) {
				...
				if ("image/jpeg".equals(tipo)) {
					post.setContent("<p>"+ rs.getString("publicacion")+"</p><p><img src='"+ rs.getString("imagen_p")+"' height='50px' width='50px'/></p>");
				}
				if ("video/mp4".equals(tipo)) {
					post.setContent("<p>"+ rs.getString("publicacion")+"</p><p> <video controls='true' autoplay='false' loop='loop' height='150px' width='300px'><source src='"+ rs.getString("imagen_p")+"' type='"+rs.getString("tipo")+"'/></video></p>");
				}
				...
				post.setContent("<p>"+ rs.getString("publicacion")+"</p><p>"
						+ " <html> "
						+ " 	<body> "
						+ " 	<center> "
